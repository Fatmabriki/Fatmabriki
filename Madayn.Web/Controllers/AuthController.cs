...
public class AuthController : BaseController
{
    ...
    private readonly Madayn.Web.Services.IProfileService _profileService;

    public AuthController(IStringLocalizer<SharedResource> localizer,
        UserManager<IdentityUser> userManager,
        SignInManager<IdentityUser> signInManager,
        Madayn.Web.Services.IProfileService profileService) : base(localizer)
    {
    ...
    public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl = null)
    {
        ...
        if (result.Succeeded)
        {
            ...
            if (user != null)
            {
                await _profileService.EnsureProfileForAspNetUserAsync(user.Id, user.Email);
            }
            return Redirect(returnUrl ?? Url.Action("Index", "Home", new { culture = GetCurrentLanguage() })!);
        }
